...
from utils.functions import remove_past_sentences, maximize_f1_score, apply_model_sentencecouple_batch, define_label
...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"results/all_data/{input_consultation_name}/{input_model_name}{('_' + input_model_revision) if input_model_revision != 'main' else ''}"):
        os.mkdir(f"results/all_data/{input_consultation_name}/{input_model_name}{('_' + input_model_revision) if input_model_revision != 'main' else ''}")

    with open(f"results/all_data/{input_consultation_name}/{input_model_name}{('_' + input_model_revision) if input_model_revision != 'main' else ''}/removepast_sentencecouple_contradictionshare.log",
    ...
    with open(f"results/all_data/{input_consultation_name}/{input_model_name}{('_' + input_model_revision) if input_model_revision != 'main' else ''}/removepast_sentencecouple_contradictionshare_metrics.log",
              "w", encoding="utf8") as file:
        for contradiction_threshold in np.append(computed_contradiction_threshold, np.arange(0.1, 1, 0.1)):
            ...
            if contradiction_threshold == computed_contradiction_threshold:
                ...
                plt.savefig(f"results/all_data/{input_consultation_name}/{input_model_name}{('_' + input_model_revision) if input_model_revision != 'main' else ''}/removepast_sentencecouple_contradictionshare_matrix.eps", format="eps")
                plt.show()
