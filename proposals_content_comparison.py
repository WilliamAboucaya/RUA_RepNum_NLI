...
proposals = pd.concat([proposals_1, proposals_2, proposals_3])

proposals["full_content"] = proposals[["contributions_title", "contributions_bodyText"]].agg(". \n\n".join, axis=1)
proposals = proposals.loc[proposals["full_content"].apply(lambda body: (len(word_tokenize(body)) > 30) & (len(word_tokenize(body)) < 254))]

proposal_titles = proposals["contributions_title"].tolist()
# proposal_contents = proposals["contributions_bodyText"].tolist()
proposal_contents = proposals["full_content"].tolist()
...
with open('results/proposals_content_comparison_RUA_with_titles.txt', 'w', encoding='utf8') as f:
